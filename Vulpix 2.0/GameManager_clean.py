...
class Gameboard():
    ...
    def checkEnergyCost(self, cost, attached):
        ...
        for i in cost:
            ...
            if i == "M":   # Has M in attack cost
                for j in range(len(test)):  # Cycles through the list of attached energies
...
#
    ...
    def retreat(self, pokemonIndex, turn):
        ...
        if turn == 'p':
            ...
            if self.playerActive[0].Retreat_Cost <= len(self.playerActive[0].Energies):
                for i in range(self.playerActive[0].Retreat_Cost):
                    self.playerDiscard.append(self.playerActive[0].Energies.pop(0))
